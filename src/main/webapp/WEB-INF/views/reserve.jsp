...
<body>
              ...
              <form class="form_horizontal">
                ...
                <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                  <div class="inline_control">
                    <p class="inline_txt selected"><span id="reservationDate"></span> 총 <span id="totalCount">0</span>매</p>
                  </div>
                </div>
              </form>
            </div>
  ...
  <div>
    <input type="hidden" id="nameCheck" readonly="readonly" value=0 />
    <input type="hidden" id="telCheck" readonly="readonly" value=0 />
    <input type="hidden" id="emailCheck" readonly="readonly" value=0 />
    <input type="hidden" id="chk3Check" readonly="readonly" value=0 />
  </div>
  <footer>
    <div class="gototop">
  ...
  <div class="group_visual">
	    ...
	    <ul class="visual_img">
	      <li class="item" style="width: 414px;"> <img alt="상품이미지" class="img_thumb" src="{{saveFileName}}"> <span class="img_bg"></span>
	        <div class="preview_txt">
	          <h2 class="preview_txt_tit">{{productDescription}}</h2> <em class="preview_txt_dsc">₩{{minimumPrice}}~ </em><em class="preview_txt_dsc">{{openingHours}}</em> </div>
	      </li>
	    </ul>
	  </div>
	</div>
	<div class="section_store_details">
      ...
      <input type="hidden" id="productId" value="{{productId}}">
	    ...
	    <p class="dsc">
	      {{#each prices}}
          {{this.priceTypeName}}석 {{this.price}}원 <br/>
        {{/each}}
	    </p>
	  </div>
	</div>
  ...
  <div class="qty">
    <input type="hidden" id="productPriceId" value="{{productPriceId}}">
    <div class="count_control">
      <!-- [D] 수량이 최소 값이 일때 ico_minus3, count_control_input에 disabled 각각 추가, 수량이 최대 값일 때는 ico_plus3에 disabled 추가 -->
      <div class="clearfix">
        <a class="btn_plus_minus spr_book2 ico_minus3 disabled" title="빼기"> </a> <input type="tel" name="count" class="count_control_input disabled" value="0" readonly title="수량">
    ...
    <div class="qty_info_icon"> <strong class="product_amount"> <span>{{priceTypeName}}석</span> </strong> <strong class="product_price"> <span class="price">{{price}}</span> <span class="price_type">원</span> </strong> <em class="product_dsc">{{price}}원 ({{discountRate}}% 할인가)</em> </div>
  </div>
  </script>
